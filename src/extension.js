...
function addPageToAppConfig(appConfigPath, pagePath) {
  try {
    ...
    if (isSubpackage && subpackageRoot) {
      ...
      // 找到 subPackages 数组
      const subPackagesRegex = /subPackages\s*:\s*\[([\s\S]*?)\](?=\s*,\s*window|\s*])/;
      const subPackagesMatch = content.match(subPackagesRegex);

      if (subPackagesMatch) {
        const subPackagesContent = subPackagesMatch[1];
        const fullMatch = subPackagesMatch[0];

        // 找到包含指定 root 的分包对象
        const escapedRoot = subpackageRoot.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const rootPattern = new RegExp('root\\s*:\\s*["\']' + escapedRoot + '["\']', 's');
        const rootIndex = subPackagesContent.search(rootPattern);
        ...
        if (rootIndex !== -1) {
          ...
          content = content.replace(fullMatch, `subPackages: [${newSubPackagesContent}]`);
